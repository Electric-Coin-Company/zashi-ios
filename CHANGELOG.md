...
### Fixed
- [MOB-1853] When sync has stalled for good and the wallet also reports a sync error, the "Sync has stalled" banner with its Retry button now takes precedence over the plain error banner, and the error details remain available in the banner's help sheet.
